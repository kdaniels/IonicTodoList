...
import { AnimationController, Gesture, GestureConfig, GestureController } from '@ionic/angular';
...
export class TaskRowComponent implements AfterViewInit {
  ...
  constructor(private gestureCtrl: GestureController, private animationCtrl: AnimationController,
    private dataService: DataService, private sanitiser: DomSanitizer) { }

  // Custom animation for side swiping task elements
    ...
    const options: GestureConfig = {
      ...
      onEnd: (ev) =>{
        ...
        if(ev.deltaX > 150){
					...
					// Close the task with the delete animation 
